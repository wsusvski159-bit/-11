...
- 同一段记忆 72 小时内不再浮现。日间浮现和自主念头取记忆时不带关键词，OB 按权重返回；情绪坐标几天不变时，每次捞到的都是同一段，同一个闪念连续几天出现、还被升成「持续念头」——那是检索单调，不是真的放不下。现在状态里记下每段记忆上次浮现的时间（`surfacedBuckets`），取材料时多要几条，剔掉 72 小时内浮现过的再取回原来的条数；全被剔掉就这一轮不浮现。
- 浮现念头挂在哪一维：按亲和度从高到低，跳过已经「涌」的维（高出自己静息线 0.05 或 ≥0.90），都在涌才挂最高的。原来永远挂亲和度最高的一维，「内心→沉淀 0.8」几乎每段记忆都带，沉淀维静息线最低、回落又慢，于是一直被念头往上堆，一直是「涌」。
